...
int32_t oa_queue_push_back(oa_queue_t* queue, const void* data) {
    ...
    if (queue->size == queue->capacity) {
        ...
        memcpy((uint8_t*)(new_data) + new_front * data_size,
               (uint8_t*)(queue->data) + queue->front * data_size, 
               (queue->capacity - queue->front) * data_size);
        free(queue->data);
        queue->data = new_data;
...
int32_t oa_queue_get_back_cp(oa_queue_t* queue, void* data) {
    ...
    data_pos = ((uint8_t*)(queue->data)) + queue->data_size * queue->back;
    switch (queue->data_size) {
    ...
    case 8: *(uint64_t*)data = *(uint64_t*)(data_pos); break;
    default: memcpy(data, data_pos, queue->data_size); break;
    }
...
int32_t oa_queue_get_front_cp(oa_queue_t* queue, void* data) {
    ...
    data_pos = ((uint8_t*)(queue->data)) + queue->data_size * queue->front;
    switch (queue->data_size) {
    case 1: *(uint8_t*)data = *(uint8_t*)(data_pos); break;
    case 2: *(uint16_t*)data = *(uint16_t*)(data_pos); break;
    case 4: *(uint32_t*)data = *(uint32_t*)(data_pos); break;
